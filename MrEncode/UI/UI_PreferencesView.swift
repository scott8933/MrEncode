...
private struct GeneralPrefs: View {
    ...
    private let defaultFilenameOrder: [FilenamePart] = [.nclc, .scale, .compression, .overlays]
    
    private func unique(_ parts: [FilenamePart]) -> [FilenamePart] {
        var seen = Set<FilenamePart>()
        var out: [FilenamePart] = []
    ...
    private func normalizeFilenameOrder(_ order: [FilenamePart]) -> [FilenamePart] {
        var normalized = unique(order)
        for part in defaultFilenameOrder where !normalized.contains(part) {
    ...
    var body: some View {
        let order = normalizeFilenameOrder(state.settings.filenameOrder)
        VStack(alignment: .leading, spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                Picker("", selection: $state.settings.uiTheme) {
                    Text("Light Mode").tag(UIThemeSelection.light)
                    Text("Dark Mode").tag(UIThemeSelection.dark)
                    Text("Follow System").tag(UIThemeSelection.system)
                }
            ...
            HStack(alignment: .center, spacing: 6) {
                ...
                Button {
                    withAnimation(.easeInOut(duration: 0.18)) {
                        state.settings.filenameOrder = [.nclc, .scale, .compression, .overlays]
                    }
                } label: {
            ...
            .onAppear {
                let normalized = normalizeFilenameOrder(state.settings.filenameOrder)
                if normalized != state.settings.filenameOrder {
                    state.settings.filenameOrder = normalized
                }
            }

            // Reorder list (Up/Down buttons; animate swaps)
            VStack(spacing: 4) {
                ForEach(Array(order.enumerated()), id: \.offset) { idx, part in
                    HStack {
    ...
    private func move(_ index: Int, up: Bool) {
        var order = normalizeFilenameOrder(state.settings.filenameOrder)
    ...
    private func sampleFragment(for part: FilenamePart) -> String {
        ...
        case .overlays:    return applySuggestedSeparator("BURNIN", settings: state.settings)
        }
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            HSplitView {
                // Left: Preset List
                VStack(alignment: .leading, spacing: 8) {
                    Text("Available Presets").font(.subheadline).foregroundColor(C.textSecondary)
                ...
                VStack(alignment: .leading, spacing: 12) {
                    if let preset = selectedPreset {
                        Text("Preset Details").font(.subheadline).foregroundColor(C.textSecondary)
                        ...
                        Text("Settings Preview").font(.subheadline).foregroundColor(C.textSecondary)
