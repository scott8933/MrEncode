...
final class PresetViewModel: ObservableObject {
    ...
    /// Imports a preset from a MrEncode droplet (.app), a preset .json file, or a compiled AppleScript .scpt.
...
private enum DropletImportError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .unsupportedSelection:
            return "Please select a MrEncode droplet (.app) or a preset .json file."
        case .presetJSONNotFound:
...
private enum DropletImporter {
    ...
    static func loadPreset(from url: URL) throws -> (Settings, String) {
        ...
        case "app":
            ...
            let resJSON = url.appendingPathComponent("Contents/Resources/mrencode_preset.json")
