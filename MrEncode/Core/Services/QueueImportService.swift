...
enum QueueImportService {
    ...
    /// *top-level only* for video files (AVFoundation-decodable). No recursive scan.
    static func process(_ urls: [URL]) -> QueueImportResult {
        ...
        for url in standardized {
            ...
            if isDir.boolValue {
                    ...
                    for child in contents {
                        if let rv = try? child.resourceValues(forKeys: Set(keys)) {
                            if rv.isDirectory == true {
                                ...
                                let p = MediaProbeService.probeVideoDecode(child)
                                if p.hasVideo && p.canDecode {
                ...
                sawAnyFile = true
                let p = MediaProbeService.probeVideoDecode(url)
                if p.hasVideo && p.canDecode {
        ...
        if topLevelMovieFiles.isEmpty && sawAnyDirectory && !sawAnyFile {
            warnings.append("No video files (.mov, .mp4, .m4v) found in the selected folder(s)")
        }
