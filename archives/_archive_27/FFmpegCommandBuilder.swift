...
enum FFmpegCommandBuilder {
        ...
        if settings.bypassHEVC {
            if !wantsScale && !wantsOverlay {
                ...
                if let trip = tripletToApply(from: settings, meta: meta) {
                    // Ask MOV muxer to write 'colr' atom and set stream color fields
                    args += ["-movflags", "write_colr",
                             "-color_primaries", trip.primaries,
                             "-color_trc",       trip.trc,
                             "-colorspace",      trip.matrix]
                }
                // No -tag:v here (keep original)
                ...
                // Keep original tags (no -tag:v override)
                args.append(output.path)
                return args
            }
        }
    ...
    private static func applyVFRPassThrough(_ args: inout [String], outputExt: String) {
        // Old + widely supported:
        args += ["-vsync", "vfr"]
        // Optional (newer ffmpeg): helps mp4/mov avoid CFR
