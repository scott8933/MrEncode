...
public var sharedMaterialBackground: some View {
    Group {
        if #available(macOS 12.0, *) {
            Rectangle().fill(.ultraThinMaterial).opacity(0.55)   // match footer
        } else {
...
// MARK: - Resizable handle used by the queue panel
struct UI_ResizeHandle: View {
    enum Style { case normal, light }
    ...
    // Notifies parent when dragging begins/ends (so it can disable scrolling)
    var onResizingChanged: ((Bool) -> Void)? = nil

    @State private var hovering = false
    @State private var dragStartHeight: CGFloat = 0
    @State private var isDragging = false

    private var fillOpacity: Double { style == .light ? (hovering ? 0.50 : 0.35)
                                                      : (hovering ? 0.75 : 0.60) }

    var body: some View {
        ZStack {
            Rectangle().fill(Color.clear).frame(height: 16).contentShape(Rectangle())
        ...
        // Win against ScrollView/List gestures
        .highPriorityGesture(
            DragGesture(minimumDistance: 0)
                .onChanged { value in
                    ...
                    // Ensure no implicit animation from the environment is applied to height
        ...
        .accessibilityLabel("Resize queue panel")
    }
}
