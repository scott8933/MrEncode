...
struct UI_CompressionOptions: View {
        ...
        GroupBox {
            DisclosureGroup(isExpanded: $state.settings.generalExpanded) {
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    // Quality + Don't Compress (checkbox styled switch at the right spot)
                    HStack(spacing: 20) {
                        ...
                        // Don't Compress (retain visual style)
                        Toggle("Don't Compress", isOn: $state.settings.bypassHEVC)
                            .toggleStyle(.switch)
                            .help("Copy video stream without HEVC re-encode when possible.")
                    ...
                    // Filename suffix (Compression) — now at the bottom
    ...
    private var exampleOutputName: String {
        ...
        return "\(base)\(state.settings.outputSuffix.isEmpty ? "-HEVC" : state.settings.outputSuffix).\(ext)"
    }
}
