        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            ZStack {
                ...
                ZStack {
                    ...
                    // Added checkbox for Queue Item now
                    List(selection: $localSelection) {
                        ForEach(state.files) { item in
                            ...
                            HStack(alignment: .firstTextBaseline, spacing: 8) {
                                // ✅ Checkbox that toggles the row’s isChecked
                                Toggle("", isOn: Binding(
                                    get: {
                                        state.files.first(where: { $0.id == item.id })?.isChecked ?? false
                                    },
                                    set: { newVal in
                                        if let idx = state.files.firstIndex(where: { $0.id == item.id }) {
                                            state.files[idx].isChecked = newVal
                                        }
                                    }
                                ))
                                .labelsHidden()
