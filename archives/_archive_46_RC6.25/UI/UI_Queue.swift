                ...
                if candidates.count > 25 {
                    ...
                    for url in candidates {
                        ...
                        if state.settings.runMode == .remoteDeadline {
                            ...
                            if !check.ok {
                                if let idx = AppCore.shared.files.firstIndex(where: { $0.url.standardizedFileURL == url.standardizedFileURL }) {
                                    AppCore.shared.files[idx].status = .blocked
                                    AppCore.shared.files[idx].statusReason = check.reason ?? "Source not accessible to render farm"
                                    AppCore.shared.files[idx].isChecked = false
                                }
                                state.pushMessage(
                                    level: .warning,
                                    "Blocked (source not visible to farm)",
                                    filename: url.lastPathComponent,
                                    code: .farmPath,
                                    originKey: "src-farm-path",
    ...
    var body: some View {
        ...
        .onChange(of: state.settings.runMode) { newMode in
            if newMode == .remoteDeadline {
                ...
                for i in AppCore.shared.files.indices {
                    let srcURL = AppCore.shared.files[i].url
                    let check = EncodeRemote.isInputPathAcceptableForFarm(srcURL)
                    if !check.ok {
                        AppCore.shared.files[i].status = .blocked
                        AppCore.shared.files[i].statusReason = check.reason ?? "Source not accessible to render farm"
                        AppCore.shared.files[i].isChecked = false
                    }
                }
            } else if newMode == .localFFmpeg {
                // Switching back to Local: unblock and re-enable encode
                for i in AppCore.shared.files.indices {
                    if AppCore.shared.files[i].status == .blocked {
                        AppCore.shared.files[i].status = .queued
                        AppCore.shared.files[i].statusReason = nil
                        AppCore.shared.files[i].isChecked = true  // enable Encode button
                    }
                }
            }
        }
