...
public var sharedMaterialBackground: some View {
    Group {
        if #available(macOS 12.0, *) {
            Rectangle().fill(AppColors.panelFill).opacity(0.55) // CHANGED: Use AppColors
        } else {
            AppColors.panelFill.opacity(0.80) // CHANGED: Use AppColors (was Color(nsColor: .windowBackgroundColor))
        }
    }
}
