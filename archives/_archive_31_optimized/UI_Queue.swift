        ...
        func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView? {
            ...
            let cell = tableView.makeView(withIdentifier: NSUserInterfaceItemIdentifier(id), owner: self) as? NSTableCellView
                ?? {
                    let c = NSTableCellView()
                    c.identifier = NSUserInterfaceItemIdentifier(id)
                    // Create a single NSHostingView that we'll reuse
            ...
            // CREATE SIMPLIFIED ROW VIEW THAT USES CACHED DATA
            let rowView = UI_QueueRow_Simplified(
                item: item,
            ...
            .environmentObject(parent.stateRef ?? AppState())

            // Update the hosting view efficiently
            if let host = cell.subviews.first as? NSHostingView<AnyView> {
