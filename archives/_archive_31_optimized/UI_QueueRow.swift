        ...
        HStack(alignment: .firstTextBaseline, spacing: 8) {
            VStack(alignment: .leading, spacing: 4) {
                ...
                if let actualSecs = item.actualEncodeSeconds, actualSecs.isFinite, actualSecs > 0 {
                    ...
                    let sizeText: String = {
                        if let cachedSize = item.cachedFileSize {
                            return formatFileSize(cachedSize)
                        } else if let finalURL = item.finalOutputURL,
                                  let finalBytes = fileSizeBytes(finalURL) {
                            return formatFileSize(finalBytes)
                        } else {
                            return "Unknown size"
                        }
                    }()
                    
                    Text("Final Size: \(sizeText) • Render Time: \(EncodeTimeEstimator.formatTime(actualSecs))")
                        .font(.callout)
                        .foregroundColor(.secondary)
                        .lineLimit(1)
                        .padding(.leading, chevronIndent)
                } else {
                    // Show estimated info without heavy computation
                    let estText = item.cachedDstLine ?? "Calculating..."
                    if !estText.isEmpty && estText != "Calculating..." {
                        Text("Est. \(estText)")
                            .font(.callout)
                            .foregroundColor(.secondary)
                            .lineLimit(1)
...
fileprivate extension UI_QueueRow {
    ...
    func makeSrcLine(url: URL, meta: MediaMetadata) -> String? {
        // Use cached data first, fall back to minimal computation
        if let cached = item.cachedSrcLine, !cached.isEmpty {
            return cached
        }
        
        // Minimal fallback - avoid AVAsset creation
        var parts: [String] = []
        if let fps = meta.nominalFPS, fps > 0 {
            parts.append("\(Int(round(fps))) fps")
        }
        if !parts.isEmpty {
    ...
    // REPLACE THE makeDstLine METHOD WITH THIS CACHED VERSION:
    func makeDstLine(url: URL, meta: MediaMetadata, settings: Settings) -> String? {
        // Use cached data first
        if let cached = item.cachedDstLine, !cached.isEmpty {
            return cached
        }
