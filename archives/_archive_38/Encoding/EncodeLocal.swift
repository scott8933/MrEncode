...
enum EncodeLocal {
    ...
    private static func encodeItem(_ item: MediaItem, settings: Settings, ffmpegPath: String) {
        ...
        DispatchQueue.main.async {
            ...
            AppState.unregisterProcess(for: item.id)
            ...
            if res.ok && finalizeOK {
                ...
                shared.files[idx].statusReason = finalizeError ?? "ffmpeg failed"
                                        ...
                                        estimate: Double?) -> (ok: Bool, cancelled: Bool, tail: String, logURL: URL?)
    {
        ...
        AppState.registerProcess(task, for: item.id)
                        ...
                        if let shared = AppState.shared,
                           let idx = shared.files.firstIndex(where: { $0.id == item.id }) {
                            ...
                            if let old = shared.files[idx].etaSeconds, old.isFinite {
                                shared.files[idx].etaSeconds = 0.7 * old + 0.3 * etaWall
                            } else {
