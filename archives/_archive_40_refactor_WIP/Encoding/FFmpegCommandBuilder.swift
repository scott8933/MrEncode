...
enum FFmpegCommandBuilder {
    ...
    // MARK: - Core
        ...
        private static func buildArgs(input: URL,
            ...
            // =========================
            // BYPASS HEVC BEHAVIOR
            // =========================
            if settings.bypassHEVC {
                if !wantsScale && !wantsOverlay {
                    ...
                    // No codec tag for passthrough
                    args.append(output.path)
                    return args
                } else {
                    // Pixel changes needed → re-encode with the ORIGINAL codec (not HEVC)
                    ...
                    // Choose encoder based on source FourCC
                    let fourcc = detectSourceCodecFourCC(input)
                    args += sameCodecEncoderArgs(forFourCC: fourcc, crf: settings.qualityCRF)

                    // Audio: copy (don't recompress)
                    args += ["-c:a", "copy"]
                    ...
                    // Keep original tags (no codec tag override)
                    args.append(output.path)
                    return args
                }
            }
            ...
            // Container-specific codec tagging
            addCodecTagging(&args, container: containerFormat)
        ...
        /// Add codec tagging based on container format
        private static func addCodecTagging(_ args: inout [String], container: ContainerFormat) {
            switch container {
            case .mov:
                // Apple playback tag for MOV
                args += ["-tag:v", "hvc1"]
            case .mp4:
                // MP4 typically uses hev1 for broader compatibility
                args += ["-tag:v", "hev1"]
            }
        }
    ...
    private static func overlayFilters(input: URL,
        ...
        // 2) FRAMES — JUST the count, zero-padded to end-frame width, honoring start offset
        if wantFr {
            ...
            // Use eif (widely supported). printf-style zero padding via %0Nd.
    ...
    /// Detect source video codec FourCC (e.g., "apch","avc1","hvc1"). Returns lowercase; empty if unknown.
    private static func detectSourceCodecFourCC(_ url: URL) -> String {
        let asset = AVAsset(url: url)
        if let v = asset.tracks(withMediaType: .video).first,
           let anyDesc = v.formatDescriptions.first {
            let desc = anyDesc as! CMFormatDescription   // CF type; cast will always succeed
            let fourcc = CMFormatDescriptionGetMediaSubType(desc)
            return fourCCString(fourcc).lowercased()
        }
        return ""
    }
