...
enum EncodeRemote {
        ...
        DispatchQueue.main.async {
            ...
            for item in items where item.status != .blocked {
                if let idx = AppCore.shared.files.firstIndex(where: { $0.id == item.id }) {
                    ...
                    AppCore.shared.files[idx].statusReason = "Submitting to Deadline…"
                    AppCore.shared.files[idx].progressMode = .fake
                    AppCore.shared.files[idx].progress = nil  // Indeterminate progress
                    AppCore.shared.files[idx].etaSeconds = nil
                    AppCore.shared.files[idx].isChecked = false  // Uncheck immediately to prevent double-submit
                }
            }
        }
            ...
            for item in items where item.status != .encoding && item.status != .blocked {
                    ...
                    DispatchQueue.main.async {
                        if let shared = AppState.shared,
                           let idx = shared.files.firstIndex(where: { $0.id == item.id }) {
                            ...
                            AppCore.shared.files[idx].statusReason = okFarm.reason
                ...
                DispatchQueue.main.async {
                    ...
                    if result.exitCode == 0 {
                        AppCore.shared.files[idx].finalOutputURL = result.output
                        AppCore.shared.files[idx].status = .queued
                        AppCore.shared.files[idx].statusReason = "Submitted to Deadline."
                        ...
                        AppCore.shared.files[idx].statusReason = msg.isEmpty
    ...
    static func submitFFmpegJob(deadlineCmd: String,
        ...
        if liveItem.meta.startTimecode == nil || liveItem.meta.nominalFPS == nil {
            ...
            if let idx = AppCore.shared.files.firstIndex(where: { $0.url == liveItem.url }) {
                DispatchQueue.main.async {
                    AppCore.shared.files[idx].meta = liveItem.meta
                }
            }
        }
