...
class FooterViewModel: ObservableObject {
    ...
    func submitButtonTitle(hasSelection: Bool, runMode: RunMode) -> String {
        let target = hasSelection ? "Selected" : "All"
        let action = runMode == .localFFmpeg ? "Encode" : "Submit"
        return "\(action) \(target)"
    }
    
    func submitButtonEnabled(files: [MediaItem], hasSelection: Bool) -> Bool {
        let targetFiles = hasSelection ? 
            files.filter { file in files.contains { $0.id == file.id } } : 
            files
        
        return targetFiles.contains { $0.isChecked && $0.status == .queued }
    }
    
    // MARK: - Cancel Button Logic
    
    func cancelButtonEnabled(files: [MediaItem]) -> Bool {
        return files.contains { $0.status == .encoding }
    }
    
    func cancelButtonTitle(files: [MediaItem]) -> String {
        let encodingCount = files.filter { $0.status == .encoding }.count
        return encodingCount > 1 ? "Cancel All (\(encodingCount))" : "Cancel"
    }
