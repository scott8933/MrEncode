...
enum EncodeLocal {
                                        ...
                                        estimate: Double?) -> Bool
    {
        ...
        // Initialize progress mode + seed ETA (if we have an estimate)
        DispatchQueue.main.async {
            if let shared = AppState.shared,
               let idx = shared.files.firstIndex(where: { $0.id == item.id }) {
                if duration != nil {
                    ...
                    shared.files[idx].progress = 0
                    // Seed with estimator so the first seconds look sane
                    if let est = estimate, est > 0 {
        ...
        errPipe.fileHandleForReading.readabilityHandler = { handle in
            ...
            // Parse ffmpeg's "time=hh:mm:ss.xx"
            if let range = chunk.range(of: #"time=\d{2}:\d{2}:\d{2}\.\d{2}"#, options: .regularExpression) {
                ...
                if let total = duration, total > 0 {
                    // Progress as media fraction (cap slightly below 1.0 until exit)
                    let prog = max(0.0, min(0.995, tSec / total))

                    // Convert media remaining -> WALL time using observed speed
                    // speed = processed_media / elapsed_wall  (media sec per real sec)
                    let elapsed = Date().timeIntervalSince(start)
                    let speed = (tSec > 0 && elapsed > 0) ? (tSec / elapsed) : 0.0
                    ...
                    DispatchQueue.main.async {
                        if let shared = AppState.shared,
                           let idx = shared.files.firstIndex(where: { $0.id == item.id }) {
                            ...
                            // Light EMA to reduce jitter in the label
                ...
                // Fake mode: elapsed / estimated wall time
