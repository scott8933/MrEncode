                ...
                VStack(alignment: .trailing, spacing: 2) {
                    ...
                    HStack(spacing: 6) {
                        ...
                        Text(state.isBackgroundProcessing ?
                             ...
                             " ")  // Single space to maintain height
                            .font(.caption)
                            .foregroundColor(.secondary)
                            .frame(minHeight: 16)  // Ensure consistent height
                    }
                    .frame(height: 20)  // Fixed height for top line
                    
                    // Bottom line: progress bar (always present, just invisible when not processing)
                    ProgressView(value: state.backgroundProgress)
                        .frame(width: 120, height: 6)  // Fixed dimensions
                        .opacity(state.isBackgroundProcessing ? 1.0 : 0.0)
                }
                .frame(width: 200, height: 28)  // Fixed container size
                .padding(.trailing, 8)
            }
