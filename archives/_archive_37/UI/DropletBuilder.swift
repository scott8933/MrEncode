...
final class DropletBuilder {
    ...
    private func encodeSettingsAsJSON(_ settings: Settings) throws -> String {
        var dropletSettings = settings.forDroplet()
        
        // Ensure all Deadline settings are complete for droplets
        dropletSettings.ensureCompleteDeadlineSettings()
        
        let dropletFile = DropletFile(presetName: "Droplet", settings: dropletSettings)
        let encoder = JSONEncoder()
        encoder.outputFormatting = []  // Use compact format (no pretty printing)
