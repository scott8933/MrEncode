...
class AppCore: ObservableObject {
            ...
            if settings.runMode == .remoteDeadline {
                ...
                if !check.ok {
                    ...
                    pushMessage(level: .warning,
                               "Blocked: \(url.lastPathComponent) — \(check.reason ?? "Path not accessible")",
            ...
            DispatchQueue.main.async {
                ...
                for (url, metadata) in results {
                    if let index = self.files.firstIndex(where: { $0.url == url }) {
                        self.files[index].meta = metadata
                        self.files[index].isProcessingMetadata = false
                        
                        // Cache file size
                        if let size = self.fileService.getFileSize(for: url) {
                            self.files[index].setCachedFileSize(size)
                        }
                        
                        // Build and cache display lines
                        let srcLine = self.fileService.buildCachedSrcLine(for: self.files[index])
                        let dstLine = self.fileService.buildCachedDstLine(for: self.files[index], settings: self.settings)
                        
                        self.files[index].setCachedSrcLine(srcLine)
                        self.files[index].setCachedDstLine(dstLine)
                    }
                }
    ...
    func markFinished(itemID: UUID, outputURL: URL) {
        if let i = files.firstIndex(where: { $0.id == itemID }) {
            files[i].finalOutputURL = outputURL
            files[i].status = .done
            files[i].statusReason = nil
        }
        checkDropletExit()
    ...
    private func updateItemStatus(itemID: UUID, status: EncodeStatus, reason: String?) {
        if let index = files.firstIndex(where: { $0.id == itemID }) {
            files[index].status = status
            files[index].statusReason = reason
    ...
    func setStatus(_ status: EncodeStatus, forIndex index: Int) {
        DispatchQueue.main.async { [weak self] in
            guard let self = self, self.files.indices.contains(index) else { return }
    ...
    func refreshDeadlineOptions(inBackground: Bool = false) {
        ...
        let dlCmd = settings.deadlineCommandPath.isEmpty
            ? EncodeRemote.detectDeadlineCommand(preferredPath: nil)
            : settings.deadlineCommandPath

        print("🔍 Using Deadline command path: \(dlCmd)")
        
        DispatchQueue.global(qos: .userInitiated).async {
            let result = EncodeRemote.fetchPoolsAndGroups(deadlineCmd: dlCmd)
            ...
            DispatchQueue.main.async {
                ...
                self.deadlineError = self.deadlineAvailable ? nil : "No pools found"
...
extension AppCore {
    ...
    func remove(id: UUID) {
        onMain {
            guard let idx = self.index(for: id) else { return }
            self.files.remove(at: idx)
        }
    }

    func toggleQueued(id: UUID, isQueued: Bool) {
        onMain {
            guard let idx = self.index(for: id) else { return }
            self.files[idx].isChecked = isQueued
        }
    }

    func setCustomDestination(id: UUID, _ url: URL?) {
        onMain {
            guard let idx = self.index(for: id) else { return }
            self.files[idx].finalOutputURL = url
        }
    }

    // MARK: - Encoding status & progress
    func setStatus(id: UUID, _ status: EncodeStatus, reason: String? = nil) {
        onMain {
            guard let idx = self.index(for: id) else { return }
            self.files[idx].status = status
            self.files[idx].statusReason = reason
            if status != .encoding { self.files[idx].progress = nil }
        }
    }

    func setProgress(id: UUID, _ progress: Double?) {
        onMain {
            guard let idx = self.index(for: id) else { return }
    ...
    func setFinalOutput(id: UUID, _ url: URL) {
        onMain {
            guard let idx = self.index(for: id) else { return }
            self.files[idx].finalOutputURL = url
        }
    ...
    func markProcessingMetadata(id: UUID, _ active: Bool) {
        onMain {
            guard let idx = self.index(for: id) else { return }
            self.files[idx].isProcessingMetadata = active
        }
    }

    // MARK: - Logs / Messages
    // Wrapper so our patches can call a stable name; forwards to your existing pushMessage(...)
    func appendLog(level: LogLevel,
                   _ message: String,
                   filename: String? = nil,
                   code: LogCode? = nil,
                   originKey: String? = nil,
                   detail: String? = nil) {
        self.pushMessage(level: level,
                         message,
                         filename: filename,
                         code: code,
                         originKey: originKey,
