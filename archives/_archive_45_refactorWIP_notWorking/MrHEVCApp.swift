...
struct MrHEVCApp: App {
    ...
    var body: some Scene {
        WindowGroup(" ") {
            ContentView()
                ...
                .onAppear {
                    // No global AppState; pass the live instance where needed.
    ...
    private func runRemoteEncodeCLI(item: MediaItem, output: URL, settings: Settings) -> Bool {
        // Submit to Deadline
        let preferred = settings.deadlineCommandPath.trimmingCharacters(in: .whitespacesAndNewlines)
        // Resolve the actual deadlinecommand path (uses preferred if provided; otherwise finds a valid path or falls back)
        let deadlineCmd = EncodeRemote.detectDeadlineCommand(preferredPath: preferred.isEmpty ? nil : preferred)
        ...
        let result = EncodeRemote.submitFFmpegJob(
            ...
            item: item,
            settings: settings,
            exportOnly: false        // no export-only toggle in Settings
        )
