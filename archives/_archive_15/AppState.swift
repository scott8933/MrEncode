...
final class AppState: ObservableObject {
    ...
    // MARK: - Compact App Log (UI_LogPane uses these)

    /// Recent messages for the Messages pane.
    @Published var logs: [AppLogEntry] = []

    /// When true, the Messages pane should expand (errors set this automatically).
    @Published var showLogPane: Bool = false

    /// Keep things tidy — don’t grow without bound.
    private let logLimit = 200

    /// Append a log entry and (optionally) auto-reveal the pane for non-info levels.
    ...
    /// Clear the visible log (used by the pane’s “Clear” button).
    func clearLogs() {
        logs.removeAll()
    }
    ...
    init() {
        ...
        // Set after settings are in place so delegate/handlers can safely use it.
    ...
    func addFiles(_ urls: [URL]) {
        for url in urls {
            ...
            // ⟵ NEW: run dimension preflight immediately (warns once per item)
            VideoPreflight.warnIfEvenizeNeeded(for: item, settings: settings)
