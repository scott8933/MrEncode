...
enum FFmpegCommandBuilder {
    ...
    private static func buildArgs(input: URL, output: URL, settings: Settings, meta: MediaMetadata) -> [String] {
        ...
        // Video filter chain (evenize+scale FIRST, then overlays)
        var filters: [String] = []
    ...
    // MARK: - Scale (always “evenize” by rounding UP to even; no borders)

    /// Returns a scale filter that applies the user scale (1×/½/¼) and *rounds up to even*,
    /// then normalizes sample aspect ratio.
    ///
    /// Examples:
    ///  - 1×:  scale=ceil(iw/2)*2 : ceil(ih/2)*2 ,setsar=1
    ///  - ½×:  scale=ceil(iw*0.5/2)*2 : ceil(ih*0.5/2)*2 ,setsar=1
    ///  - ¼×:  scale=ceil(iw*0.25/2)*2 : ceil(ih*0.25/2)*2 ,setsar=1
    ///
    /// Using `ceil(.../2)*2` avoids shrinking by 1px when inputs are odd.
    private static func scaleFilter(for scale: ScaleOption) -> String? {
        switch scale {
        case .oneToOne:
            // Evenize at native size, no visible border (may resample by ≤1px)
    ...
    private static func overlayFilters(for settings: Settings, input: URL, meta: MediaMetadata) -> [String] {
        ...
        // Filename (full basename INCLUDING extension)
