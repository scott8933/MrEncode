...
struct UI_QueueRow: View {
    ...
    @State private var basics: MediaBasics? = nil
    ...
    var body: some View {
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                if isExpanded {
                    ...
                    let dstText: String = {
                        ...
                        // Estimated output bitrate and file size (fast path through your estimator)
                        if let estimate = OutputEstimator.estimate(url: item.url, meta: item.meta, settings: state.settings) {
        ...
        .onAppear {
            MediaProbe.probeAsync(url: item.url,
                                  meta: item.meta,
                                  scale: state.settings.scale) { result in
                self.basics = result
                if let result,
                   let idx = AppCore.shared.files.firstIndex(where: { $0.id == item.id }) {
                    if AppCore.shared.files[idx].meta.nominalFPS == nil {
                        AppCore.shared.files[idx].meta.nominalFPS = result.fps
                    }
                    if AppCore.shared.files[idx].meta.durationSeconds <= 0 {
                        AppCore.shared.files[idx].meta.durationSeconds = result.duration
                    }
                }
            }
        }
...
fileprivate extension UI_QueueRow {
    ...
    func makeDstTechnicalSpecs(url: URL, meta: MediaMetadata, settings: Settings) -> String? {
        ...
        // If user selected "No Recompression", reflect that explicitly and stop here.
