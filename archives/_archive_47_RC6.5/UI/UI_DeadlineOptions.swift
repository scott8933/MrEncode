    ...
    var body: some View {
        GroupBox {
            DisclosureGroup(isExpanded: $state.settings.deadlineExpanded) {
                ...
                // --- Sticky persistence hooks (1B) ---
                .onChange(of: state.settings.pool) { _ in
                    PreferencesService.shared.updateSticky(from: state.settings)
                }
                .onChange(of: state.settings.secondaryPool) { _ in
                    PreferencesService.shared.updateSticky(from: state.settings)
                }
                ...
                .onAppear {
                    // Hydrate any missing/“none” values from machine-local stickies
