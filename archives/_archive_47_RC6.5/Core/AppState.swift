...
final class AppState: ObservableObject {
    ...
    func submit() {
        let itemsToSubmit = settings.autoEncodeOnDrop
            ? files.filter { $0.status == .queued }                 // Auto: all runnable
            : files.filter { $0.status == .queued && $0.isChecked } // Manual: only checked
    ...
    func exportDroplet(name: String) {
        let s = settings
        if s.runMode == .remoteDeadline && !s.deadlinePoolsValid {
            pushMessage(
                level: .warning,
        ...
        DropletBuilder.shared.showCreateDropletDialog(presetName: name, settings: s) { _ in }
    }
