...
enum EncodeRemote {
    ...
    static func writeFFmpegJobBundlesToDesktop(items: [MediaItem],
        ...
        for item in items {
            ...
            let full = FFmpegCommandBuilder.buildArgs(input: input, output: output, settings: settings)
            let outputArgs = ffmpegOutputArgs(fromFullArgs: full, input: input, output: output)
    ...
    static func submitFFmpegJob(deadlineCmd: String,
        ...
        // Pair paths
        let baseStem = output.deletingPathExtension().lastPathComponent
        let jobInfoURL    = tmpDir.appendingPathComponent("\(baseStem)_jobInfo.job")
        let pluginInfoURL = tmpDir.appendingPathComponent("\(baseStem)_pluginInfo.job")

        // Write the FFmpeg plugin pair
        do {
            try writeFFmpegJobInfo(to: jobInfoURL, input: input, output: output, settings: settings)
            try writeFFmpegPluginInfo(to: pluginInfoURL,
                                      input: input,
            ...
            // Couldn’t write job files
            return SubmissionResult(input: input, output: output, exitCode: -1,
                                    rawOutput: "Failed to write job files: \(error)")
        }
