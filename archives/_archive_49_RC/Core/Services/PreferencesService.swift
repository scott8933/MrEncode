...
final class PreferencesService {
    ...
    func loadSettings() -> Settings {
        do {
            ...
            print("⚠️ Preferences load failed, attempting factory defaults: \(error)")
            do {
                let defaults = try store.resetUserSettingsToFactoryDefaults()
                return normalize(defaults)
            } catch {
                fatalError("Unable to load MrHEVC preferences or factory defaults: \(error)")
            }
        }
    }
    ...
    func resetSettingsToFactoryDefaults() -> Settings {
        do {
            ...
            fatalError("Failed to reset MrHEVC preferences to factory defaults: \(error)")
        }
    }
