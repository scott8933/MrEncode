...
enum Buckets {
    ...
    static func fpsBucket(_ fps: Double) -> Int {
        ...
        let best = targets.min { abs($0 - fps) < abs($1 - $0) } ?? 30
...
// MARK: - Codec key helper (for bucket ids)
enum CodecKey {
...
final class EncodeStatsStore {
    ...
    private let queue = DispatchQueue(label: "mrencode.encodestats", qos: .utility)
    ...
    /// Legacy key (codec-agnostic) — kept for back-compat and fallback
    ...
    /// New codec-aware key (preferred)
    static func makeKey(runMode: String,
                        res: String,
                        fps: Int,
                        crf: Int,
                        codecKey: String,
                        isHDR: Bool? = nil) -> String {
        "\(runMode)|\(res)|fps\(fps)|crf\(crf)|codec:\(codecKey)\(Buckets.hdrSuffix(isHDR))"
    }

    // MARK: Public: encode-time (MP/s)

    /// Legacy record using legacy (codec-agnostic) key — keep for existing call sites
    ...
    /// New: record MP/s with codec-aware bucket
    func record(sample: EncodeSample, codec: VideoCodec, isHDR: Bool? = nil) {
        queue.async {
            let key = Self.makeKey(runMode: sample.runMode,
                                   res: sample.resBucket,
                                   fps: sample.fpsBucket,
                                   crf: sample.crfBucket,
                                   codecKey: CodecKey.from(codec),
                                   isHDR: isHDR)
    ...
    /// New: codec-aware average, with fallback to legacy key if missing
    func averageMPps(runMode: String,
                     outW: Int, outH: Int,
                     fps: Double,
                     crf: Int,
                     codec: VideoCodec,
                     isHDR: Bool? = nil) -> Double? {
        let res = Buckets.resBucket(outW, outH)
        let fb  = Buckets.fpsBucket(fps)
        let cb  = Buckets.crfBucket(crf)
        let ck  = CodecKey.from(codec)
        let kNew = Self.makeKey(runMode: runMode, res: res, fps: fb, crf: cb, codecKey: ck, isHDR: isHDR)
        let kOld = Self.makeKey(runMode: runMode, res: res, fps: fb, crf: cb, isHDR: isHDR)
        return queue.sync {
            if let s = payload.buckets[kNew] { return s.emaMPps }
            if let s = payload.buckets[kOld] { return s.emaMPps }
            return payload.overallEMA
        }
    }
    ...
    /// Legacy record for bpppf@CRF18 (codec-agnostic)
    ...
    /// New: codec-aware record for bpppf@CRF18
    func recordBpppf18(runMode: String,
                       outW: Int, outH: Int,
                       fps: Double,
                       crf: Int,
    ...
    /// Legacy average for bpppf@CRF18 (codec-agnostic)
    ...
    /// New: codec-aware average for bpppf@CRF18, with fallback to legacy key
    func averageBpppf18(runMode: String,
                        outW: Int, outH: Int,
                        fps: Double,
                        crf: Int,
        ...
        let kOld = Self.makeKey(runMode: runMode, res: res, fps: fb, crf: cb, isHDR: isHDR)
        return queue.sync {
            if let v = payload.buckets[kNew]?.emaBpppf18 { return v }
            return payload.buckets[kOld]?.emaBpppf18
        }
    }
