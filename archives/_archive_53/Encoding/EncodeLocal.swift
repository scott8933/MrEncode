...
private enum TempOut {
    static let q = DispatchQueue(label: "mrencode.local.tempouts", attributes: .concurrent)
...
enum EncodeLocal {
    ...
    static func run(items: [MediaItem], settings: Settings, ffmpegPath: String? = nil) {
        ...
        // Process items serially (one local encode at a time)
        for item in items {
    ...
    private static func encodeItem(_ item: MediaItem, settings: Settings, ffmpegPath: String) {
        ...
        // Planned destination is authoritative for the NEXT encode attempt.
        // Do NOT reuse finalOutputURL (that is the last successful output, used for Reveal/history).
        let outputURL: URL = {
            // Use the snapshot copy first (cheap)
            if let planned = item.plannedOutputURL { return planned }

            // Then ask the main thread for the authoritative row value (avoid stale copies)
            var result: URL?
        ...
        if CompressorTagger.tryTagOnlyIfEligible(input: item.url,
            ...
            DispatchQueue.main.async {
                if let shared = AppState.shared,
                   let i = shared.files.firstIndex(where: { $0.id == item.id }) {
                    AppCore.shared.files[i].finalOutputURL = outputURL
                    AppCore.shared.files[i].status = .done
                    AppCore.shared.files[i].statusReason = nil
                    AppCore.shared.files[i].actualEncodeSeconds = tagFinish.timeIntervalSince(tagStart)
                    AppCore.shared.files[i].progress = nil
                    AppCore.shared.files[i].etaSeconds = nil
                    AppCore.shared.files[i].progressMode = .none
                }
            }
        ...
        // Best-effort warm-up for MediaBasics so learning can attribute throughput correctly.
        // Do not await here (encode must not be gated by probing).
        let basicsWarmTask: Task<MediaBasics?, Never> = Task.detached(priority: .utility) {
            let t = await MediaProbeService.shared.probeBasics(url: item.url, meta: item.meta, scale: settings.scale)
        ...
        // Estimate is optional; EncodeTimeEstimator is probe-free.
        // If basics happen to be available later, UI ETA will still be refined by ffmpeg speed in REAL mode.
        let est = EncodeTimeEstimator.estimateSeconds(
            basics: nil,
            meta: item.meta,
            settings: settings,
        ...
        // Attempt to retrieve basics for learning without blocking finalize/UI.
        // If the probe is slow or cancelled, we skip learning safely.
        let basicsForLearning = Self.awaitWithTimeout(basicsWarmTask, seconds: 0.5) ?? nil

        if res.ok && finalizeOK {
            Self.learnForCompleted(
                item: item,
                settings: settings,
                startedAt: startedAt,
                finishedAt: finishedAt,
                basics: basicsForLearning
            )
        }
        ...
        DispatchQueue.main.async {
            ...
            // Persist per-item log path for later (UI "Reveal Log", etc.)
            AppCore.shared.files[idx].logURL = res.logURL

            if res.cancelled {
                // You said: on Cancel we should clear item (remove from queue)
                // If you'd rather keep the row, switch to .queued + reason like before.
                AppCore.shared.files.remove(at: idx)
            ...
            if res.ok && finalizeOK {
                ...
                AppCore.shared.localEncodeDidComplete(
                    itemID: item.id,
                    outputURL: nil,
                    errorText: finalizeError ?? "ffmpeg failed",
                    encodeSeconds: nil
                )
                ...
                AppState.shared?.pushMessage(
                    ...
                    filename: item.url.lastPathComponent,
                    detail: res.tail.isEmpty
                        ? (finalizeError ?? "ffmpeg exited with an error.")
                        : res.tail,
                    logURL: res.logURL
                )
            }
                                        ...
                                        estimate: Double?) -> (ok: Bool, cancelled: Bool, tail: String, logURL: URL?)
    {
        ...
        DispatchQueue.main.async {
            if let shared = AppState.shared,
               let idx = shared.files.firstIndex(where: { $0.id == item.id }) {
                if duration != nil {
                    AppCore.shared.files[idx].progressMode = .real
                    AppCore.shared.files[idx].progress = 0
                    AppCore.shared.files[idx].etaSeconds = estimate
                } else if let est = estimate, est > 0 {
                    AppCore.shared.files[idx].progressMode = .fake
                    AppCore.shared.files[idx].progress = 0
                    AppCore.shared.files[idx].etaSeconds = est
                } else {
                    AppCore.shared.files[idx].progressMode = .none
                    AppCore.shared.files[idx].progress = nil
                    AppCore.shared.files[idx].etaSeconds = nil
                   ...
                   let total = duration, total > 0 {
                    ...
                    DispatchQueue.main.async(qos: .background) {
                        if let shared = AppState.shared,
                           let idx = shared.files.firstIndex(where: { $0.id == item.id }) {
                            AppCore.shared.files[idx].progressMode = .real
                            AppCore.shared.files[idx].progress = prog
                            // Light EMA to reduce jitter
                            if let old = shared.files[idx].etaSeconds, old.isFinite {
                                AppCore.shared.files[idx].etaSeconds = 0.7 * old + 0.3 * etaWall
                            } else {
                                AppCore.shared.files[idx].etaSeconds = etaWall
                            }
                        }
                    }
                    ...
                    DispatchQueue.main.async(qos: .background) {
                        if let shared = AppState.shared,
                           let idx = shared.files.firstIndex(where: { $0.id == item.id }) {
                            AppCore.shared.files[idx].progressMode = .fake
                            AppCore.shared.files[idx].progress = prog
                            AppCore.shared.files[idx].etaSeconds = remain
                        }
                    }
                }
            }
    ...
    /// Await a Task result with a short timeout so we never hang finalize/UI updates.
    private static func awaitWithTimeout<T>(_ task: Task<T, Never>, seconds: TimeInterval) -> T? {
        var result: T?
        let sem = DispatchSemaphore(value: 0)

        // Inherit caller priority (or explicitly use .userInitiated)
        Task { // inherits current task/actor priority when possible
    ...
    private static func learnForCompleted(item: MediaItem,
        ...
        if let out = item.finalOutputURL,
           ...
           let bytes = attrs[.size] as? NSNumber {
            _ = bytes // hook size learning here if/when you add it
        }
    }
}
