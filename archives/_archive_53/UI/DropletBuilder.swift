// =============================
// File: DropletBuilder.swift - Cleaned up version with simplified icon handling
// =============================
...
final class DropletBuilder {
    ...
    func createDroplet(presetName: String, settings: Settings, at outputURL: URL, extraCLIArgs: [String]) throws {
        ...
        // 5. Set custom icon - with better error handling
        do {
    ...
    func showCreateDropletDialog(presetName: String, settings: Settings, completion: @escaping (Bool) -> Void) {
        ...
        panel.begin { response in
            ...
            // Accessory checkboxes (persisted)
            // NOTE: If you already added accessoryView earlier in this function, remove one copy.
            // If you haven't added it yet, keep the below accessory construction ABOVE panel.begin.
            // (See note at bottom.)

            // Read checkbox states from the accessory view
            // (Assumes you created chkPopup/chkChime in the outer scope.)
    ...
    private func setDropletIcon(at dropletURL: URL) throws {
        ...
        // Method 1: Copy .icns file into bundle (most reliable)
        ...
        // Method 2: If we have an NSImage, convert it to .icns and save it
        ...
        // Also set the icon on the bundle itself programmatically
    ...
    private func findDropletIcon() -> NSImage? {
        ...
        // Priority 4: Fall back to main app icon
        if let appIcon = NSImage(named: "AppIcon") {
            print("✅ Using main AppIcon as fallback")
            return appIcon
        }
        
        // Priority 5: Get app bundle icon as last resort
    ...
    private func updateDropletInfoPlist(at dropletURL: URL) throws {
        ...
        // CRITICAL: Set the icon file reference
        plist["CFBundleIconFile"] = "droplet"  // No .icns extension
        plist["CFBundleIconName"] = "droplet"  // Also set this for good measure
        
        // Set app identification metadata
        plist["CFBundleName"] = (plist["CFBundleName"] as? String) ?? "MrEncode Droplet"
        
        // IMPORTANT: Set LSUIElement to prevent dock icon but don't set NSUIElement
        plist["LSUIElement"] = true
        
        // Remove any NSUIElement key if it exists (can interfere with icon display)
    ...
    private func setIconProgrammatically(at dropletURL: URL) throws {
        ...
        guard let icon = findDropletIcon() else {
            print("⚠️ Could not find any icon for droplet")
            throw DropletBuilderError.fileSystemError("Could not find droplet icon")
        }
        
        workspace.setIcon(icon, forFile: dropletURL.path, options: [])
        print("✅ Set droplet icon programmatically")
    }
    
    /// Convert NSImage to .icns format and save to disk
    private func saveAsICNS(image: NSImage, to url: URL) throws {
        // Get the best representation we have
        guard let tiffData = image.tiffRepresentation,
              let bitmapRep = NSBitmapImageRep(data: tiffData) else {
            throw DropletBuilderError.fileSystemError("Could not get image representation")
        }
        
        // For simplicity, we'll save as PNG with proper size
        // macOS can use PNG files for icons in many contexts
        // However, for best results with droplets, we really want .icns
        
        // Try to create a proper .icns with iconutil if we can
        let tempDir = FileManager.default.temporaryDirectory
        ...
        // Fallback: Just save as PNG (not ideal but better than nothing)
