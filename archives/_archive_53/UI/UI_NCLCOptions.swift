...
struct UI_NCLCOptions: View {
    ...
    var body: some View {
        let inactive =
            state.settings.nclcTag.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == "no change"
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            Button {
                ...
                HStack(spacing: 8) {
                    Image(systemName: state.settings.nclcExpanded ? "chevron.down" : "chevron.right")
                        .font(.system(size: 12, weight: .semibold))
                        .foregroundColor(.secondary)
                        .frame(width: 14, height: 14)

                    let dimHeader = !state.settings.nclcExpanded && inactive
