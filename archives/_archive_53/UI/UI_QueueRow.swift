...
struct UI_QueueRow: View {
    ...
    private var sourceLinkColor: Color {
        hoveringSource ? StyleConstants.linkHover : StyleConstants.linkNormal
    }
                ...
                HStack(alignment: .firstTextBaseline, spacing: 8) {
                    ...
                    Toggle("", isOn: isQueuedBinding)
                        ...
                        .help(isMissingSourceBlocked
                              ? "This item is blocked because its source is missing. Click the filename to reload."
                    ...
                    Button {
                        ...
                        Text(item.url.lastPathComponent.isEmpty ? "(missing source)" : item.url.lastPathComponent)
                            ...
                            .contentShape(Rectangle()) // ensures the text area is a reliable hit target
                    }
                    .buttonStyle(.plain)
                    ...
                    // Empty-space hit zone for row selection (does not steal clicks from controls)
                    Color.clear
                ...
                if item.status == .encoding {
                    ...
                    if item.status == .queued, item.warningReason != nil {
                        ...
                        (Text("Queued") + Text(" (warning)").foregroundColor(.orange))
                            .font(.caption)
                            .foregroundColor(.secondary) // applies to the "Queued" portion
...
private struct CustomDestinationLink: View {
    ...
    private var linkColor: Color {
        if isEditable {
            return hovering ? StyleConstants.linkHover : StyleConstants.linkNormal
        } else {
            return item.status == .done
                ? (hovering ? StyleConstants.linkHover : StyleConstants.linkNormal)
                : StyleConstants.linkDisabled
        }
    }
