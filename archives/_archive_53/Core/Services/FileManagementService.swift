...
class FileManagementService: ObservableObject {
    ...
    func buildCachedDstLine(for item: MediaItem, settings: Settings, runMode: RunMode) -> String {
        let url = item.url
        let meta = item.meta
        
        // Generate proper estimates
        let estWallTime: Double? = EncodeTimeEstimator.estimateSeconds(basics: nil, meta: meta, settings: settings, runMode: runMode)
        
        var parts: [String] = []
        
        // Add size estimate
        if let est = OutputEstimator.estimate(basics: nil, meta: meta, settings: settings) {
