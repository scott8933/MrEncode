...
enum MetadataExtractor {
    ...
    /// Synchronous: AVFoundation first; ExifTool fallback if available.
    ...
    static func extract(for url: URL) -> MediaMetadata {
        ...
        if out.startTimecode == nil || out.nominalFPS == nil ||
            ...
            // ExifTool fallback (prefer filling missing fields only; never probe AVFoundation here)
            if let exif = ExifToolProbe.extract(for: url) {
                if Task.isCancelled { return out }

                // Timecode
                if out.startTimecode == nil, let tc = exif.timecode, !tc.isEmpty {
                    out.startTimecode = tc
                }

                // FPS
                if out.nominalFPS == nil, let f = exif.fps, f.isFinite, f > 0 {
                    out.nominalFPS = f
                }

                // Map Exif values (name OR number) → numeric code string
                if out.colorPrimaries == nil, let s = exif.colorPrimaries, !s.isEmpty {
                    out.colorPrimaries = mapNCLCCode(s, kind: .primaries) ?? s
                }
                if out.transferFunction == nil, let s = exif.transferFunction, !s.isEmpty {
                    out.transferFunction = mapNCLCCode(s, kind: .transfer) ?? s
                }
                if out.ycbcrMatrix == nil, let s = exif.matrixCoefficients, !s.isEmpty {
                    out.ycbcrMatrix = mapNCLCCode(s, kind: .matrix) ?? s
                }

                // Duration: only override if we don't already have a positive AVF duration
                if out.durationSeconds <= 0, let d = exif.durationSeconds, d.isFinite, d > 0 {
