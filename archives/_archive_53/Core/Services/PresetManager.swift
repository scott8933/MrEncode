...
final class PresetManager {
    ...
    func loadPresets() -> [EncodingPreset] {
        do {
            ...
            // Load as raw JSON first, merge defaults into each preset.settings, then decode.
            let obj = try JSONSerialization.jsonObject(with: data, options: [])
            guard let arr = obj as? [[String: Any]] else {
                throw NSError(domain: "PresetManager", code: 4,
            ...
            for presetObj in arr {
                ...
                out.append(EncodingPreset(name: name, settings: settings))
            }
