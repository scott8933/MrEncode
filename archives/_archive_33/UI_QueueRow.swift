    ...
    var body: some View {
        ...
        HStack(alignment: .firstTextBaseline, spacing: 8) {
            VStack(alignment: .leading, spacing: 4) {
                ...
                // ADD THIS PROCESSING INDICATOR HERE:
                ...
                // Main-view tertiary line: Final/Est size + Render time (use stats-based estimates)
                if let actualSecs = item.actualEncodeSeconds, actualSecs.isFinite, actualSecs > 0,
                   ...
                   let finalBytes = fileSizeBytes(finalURL) {
                    ...
                    // Use cached estimates or show processing
                    if item.isProcessingMetadata {
                        Text("Calculating estimates...")
                            ...
                            .lineLimit(1)
                            .padding(.leading, chevronIndent)
                    } else if let cached = item.cachedDstLine, !cached.isEmpty && cached != "Estimating..." {
                        Text("Est. Final Size: \(cached)")
                            .font(.callout)
                            .foregroundColor(.secondary)
                            .lineLimit(1)
                            .padding(.leading, chevronIndent)
...
fileprivate extension UI_QueueRow {
    ...
    func makeSrcLine(url: URL, meta: MediaMetadata) -> String? {
        ...
        if let cached = item.cachedSrcLine, !cached.isEmpty && cached != "Processing..." {
            return cached
        }
        
        // Build comprehensive source info including NCLC
        ...
        // Add NCLC display from source metadata
        if let nclc = nclcDisplayNow(meta: meta) { parts.append(nclc) }
    ...
    func makeDstLine(url: URL, meta: MediaMetadata, settings: Settings) -> String? {
        // Use cached data first if available
        if let cached = item.cachedDstLine, !cached.isEmpty && cached != "Estimating..." {
            return cached
        }
        ...
        // Show predicted NCLC label (what it will be after encoding)
        ...
        return parts.isEmpty ? "Estimating..." : parts.joined(separator: bullet)
    }
    ...
    struct UI_QueueRow_Simplified: View {
        @EnvironmentObject var state: AppState
        let item: MediaItem
        let suggested: URL
        var isExpandedExternal: Bool? = nil
        ...
        private var isExpanded: Bool { isExpandedExternal ?? false }
        
        var body: some View {
            HStack(alignment: .firstTextBaseline, spacing: 8) {
                VStack(alignment: .leading, spacing: 4) {
                    // Header row - minimal info only
                    HStack(alignment: .firstTextBaseline, spacing: 6) {
                        Button {
                            onToggleExpand?()
                        } label: {
                    ...
                    // Target filename
                    Text((item.finalOutputURL ?? suggested).lastPathComponent)
                        .font(.callout)
                        .foregroundColor(.secondary)
                        .lineLimit(1)
                        .padding(.leading, 20)
                    
                    // Use cached info when available
                    if let cached = item.cachedSrcLine, !cached.isEmpty {
                        Text("Final: \(cached)")
                            .font(.callout)
                            .foregroundColor(.secondary)
                            .lineLimit(1)
                    ...
                    // Details when expanded - use cached data
                    if isExpanded {
...
// ADD TO THE END OF UI_QueueRow.swift:
struct UI_QueueRow_Simplified: View {
    @EnvironmentObject var state: AppState
    let item: MediaItem
    let suggested: URL
