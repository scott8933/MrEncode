...
final class AppState: ObservableObject {
    ...
    func addFiles(_ urls: [URL]) {
        for url in urls {
            // FIX 1: put meta: before status: to match MediaItem's property order
            var item = MediaItem(
                url: url,
                meta: .empty,
                status: .queued,
            ...
            if settings.runMode == .remoteDeadline {
                ...
                if !check.ok {
                    // FIX 2 (optional): if you still see inference error, qualify the enum:
                    // item.status = EncodeStatus.blocked   // if your enum is EncodeStatus
                    item.status = .blocked                  // this is fine once the init compiles
