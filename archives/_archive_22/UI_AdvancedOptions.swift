...
struct UI_AdvancedOptions: View {
    ...
    var body: some View {
        GroupBox {
            DisclosureGroup(isExpanded: $state.settings.overlaysExpanded) {   // <-- use overlaysExpanded
                ...
                Text("Overlays").font(.headline)   // panel name now matches new design
            }
        }
        // Color sync with Settings (same behavior as before)
        .onAppear {
