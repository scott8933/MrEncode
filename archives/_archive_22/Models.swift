...
struct Settings: Codable {
    ...
    // MARK: Panel state
    var generalExpanded: Bool  = true     // "Compression & Resizing"
    ...
    /// Appears AFTER NCLC label in the filename, e.g. "-HEVC"
    var outputSuffix: String = "-HEVC"

    // MARK: NCLC Settings
    /// Picker label (pass-through when "No Change")
    var nclcTag: String = Self.nclcOptionOrderTopFirst.first ?? "No Change"
    /// Optional color/gamut label that appears BEFORE `outputSuffix` in filename (e.g. "BT709")
    ...
    var poolOptions: [String] = [] {
        didSet {
            ...
            if let first = poolOptions.first, (pool.isEmpty || !poolOptions.contains(pool)) {
                pool = first
            }
        }
    }
...
extension Settings {
    ...
    mutating func coerceDropdownDefaultsTopFirst() {
        ...
        if let firstPool = poolOptions.first, (pool.isEmpty || !poolOptions.contains(pool)) {
            pool = firstPool
        }
