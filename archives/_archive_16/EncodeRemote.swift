...
enum EncodeRemote {
            ...
            for item in items {
                    ...
                    DispatchQueue.main.async {
                        if let shared = AppState.shared,
                           let idx = shared.files.firstIndex(where: { $0.id == item.id }) {
                            ...
                            // concise warning
                            shared.log(.warning, okFarm.reason ?? "Not accessible to render farm.", fileURL: item.url)
                        }
                    }
                    continue
                }
                ...
                DispatchQueue.main.async {
                    ...
                    if result.exitCode == 0 {
                        ...
                        // concise success w/ JobID if available
                        if let job = parseJobID(from: result.rawOutput) {
                            shared.log(.info, "Submitted. JobID \(job)", fileURL: item.url)
                        } else {
                            shared.log(.info, "Submitted.", fileURL: item.url)
                        }
                    } else {
                        ...
                        let final = msg.isEmpty ? "Deadline submit failed (code \(result.exitCode))." : msg
                        shared.files[idx].statusReason = final
                        // concise failure line
                        shared.log(.error, final, fileURL: item.url, autoReveal: true)
                    }
                }
            }
        }
    ...
    static func isInputPathAcceptableForFarm(_ url: URL) -> (ok: Bool, reason: String?) {
        ...
        if path.hasPrefix(home + "/") && !path.hasPrefix("/Volumes/") {
            return (true, "Path is under your home folder; ensure render nodes can reach it.")
        }

        return (true, nil)
    }

    // MARK: - Parse JobID from deadlinecommand output (best-effort)
    private static func parseJobID(from output: String) -> String? {
        // Common patterns:
        //  - "Job submitted successfully: ... JobID=63d9e1f6b1c8d1234567890a"
        //  - Or a bare hex-ish id on its own line
        if let r = output.range(of: #"JobID=([A-Za-z0-9\-]+)"#, options: .regularExpression) {
            let s = String(output[r])
            if let eq = s.firstIndex(of: "=") {
                return String(s.suffix(from: output.index(after: eq)))
            }
        }
        if let r2 = output.range(of: #"[0-9a-fA-F]{8,}"#, options: .regularExpression) {
            return String(output[r2])
        }
        return nil
    }
}
