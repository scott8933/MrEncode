            ...
            for item in items where item.status != .encoding && item.status != .blocked {
                ...
                if !okFarm.ok {
                    DispatchQueue.main.async {
                        AppCore.shared.updateFile(id: item.id) { file in
                            file.status = .blocked
                            file.statusReason = okFarm.reason
                ...
                DispatchQueue.main.async {
                    ...
                    guard let index = AppCore.shared.files.firstIndex(where: { $0.id == item.id }) else { return }

                    if AppCore.shared.files[index].status == .blocked {
