...
class ValidationService {
    ...
    func validateItem(_ item: inout MediaItem, settings: Settings) {
        ...
        if settings.runMode == .remoteDeadline {
            ...
            if !okFarm.ok {
                item.status = .blocked
                item.statusReason = okFarm.reason ?? "Source not accessible to render farm"
        ...
        if item.status == .blocked {
            if let reason = item.statusReason, (
                ...
                reason.hasPrefix("Source not accessible to render farm") // clears when switching to Local or path now OK
            ) {
                item.status = .queued
