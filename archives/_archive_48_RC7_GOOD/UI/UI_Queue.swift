                            ...
                            if !check.ok {
                                if let idx = AppCore.shared.files.firstIndex(where: { $0.url.standardizedFileURL == url.standardizedFileURL }) {
                                    AppCore.shared.updateFile(at: idx) { file in
                                        file.status = .blocked
                                        file.statusReason = check.reason ?? "Source not accessible to render farm"
                        ...
                        onToggleQueued: { newVal in
                            ...
                            if let idx = AppCore.shared.files.firstIndex(where: { $0.id == item.id }) {
                                ...
                                if newVal,
                                   ...
                                   AppCore.shared.files.indices.contains(idx) {
                                    let refreshed = AppCore.shared.files[idx]
        ...
        .onChange(of: state.settings.runMode) { newMode in
            if newMode == .remoteDeadline {
                ...
                for i in AppCore.shared.files.indices {
                    ...
                    if !check.ok {
                        AppCore.shared.updateFile(at: i) { file in
                            file.status = .blocked
                            file.statusReason = check.reason ?? "Source not accessible to render farm"
                            file.isChecked = false
                        }
