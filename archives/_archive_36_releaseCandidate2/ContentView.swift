...
struct ContentView: View {
    ...
    var body: some View {
            ...
            if state.settings.autoEncodeOnDrop {
                ...
                ScrollView {
                    VStack(spacing: 16) {
                        ...
                        // Options panels
                        UI_PresetsDroplets().environmentObject(state)
                    ...
                    .padding(C.pad)
                    .padding(.bottom, C.actionBarHeight + C.pad + 80)  // Simple fix: add 80pt for message area
                }
            }
        }
        ...
        .onChange(of: state.settings.runMode)             { _ in state.revalidateFilesForCurrentMode() }
