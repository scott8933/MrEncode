    ...
    var body: some View {
        ...
        HStack(alignment: .firstTextBaseline, spacing: 8) {
            VStack(alignment: .leading, spacing: 4) {
                ...
                HStack(alignment: .firstTextBaseline, spacing: 8) {
                    // NEW: “Queued” checkbox (independent of List selection)
                ...
                if item.status == .encoding {
                    ...
                    ZStack(alignment: .trailing) {
                        ...
                        HStack(alignment: .center, spacing: 8) {
                            ...
                            // Right-side actions / status  (no UIQueueRowInfo)
                            HStack(spacing: 10) {
                                if item.status == .encoding {
            ...
            // Right-side actions / status  (replace your existing trailing HStack)
            HStack(spacing: 10) {
                if item.status == .encoding {
                    Button("Cancel") { state.cancelAllEncoding() }
                        .buttonStyle(.borderless)
                    ...
                    Text(status.label)                 // <- uses helper below
                        .font(.caption)
