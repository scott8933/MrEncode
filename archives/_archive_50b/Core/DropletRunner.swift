...
struct DropletRunner {
    static func run(arguments: [String]) -> Int {
        ...
        if let result = parsePathArgument(arguments, flag: "--preset-file") {
            let videoFilePaths = Array(arguments[result.nextIndex...]).filter { !$0.hasPrefix("--") }
            return runDropletCLI(dropletFile: result.path, videoFiles: videoFilePaths)
        }
    ...
    private static func runDropletCLI(dropletFile: String, videoFiles: [String]) -> Int {
        ...
        for (index, entry) in results.enumerated() {
            ...
            case .failure(let error):
                if case .cancelled = error {
                    ...
                    print("  Failed")
                    presentCLI(error: error, fileName: fileURL.lastPathComponent)
                }
            }
        }
        ...
        if cancelCount > 0 {
            print("Cancelled: \(cancelCount)")
        }
        if !submittedJobIDs.isEmpty {
            print("Job IDs: \(submittedJobIDs.joined(separator: ", "))")
        }
    ...
    private static func presentCLI(error: DropletError, fileName: String) {
    ...
    static func presentGUI(error: DropletError, fileURL: URL, state: AppState) {
        state.pushMessage(
    ...
    private static func showCLIErrorDialog(title: String, message: String, detail: String) {
        DispatchQueue.main.async {
            let alert = NSAlert()
