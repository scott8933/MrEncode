...
struct UI_HeaderBar: View {
    ...
    /// Kept for API compatibility; not used anymore.
    ...
    var body: some View {
        HStack(spacing: StyleConstants.headerSpacing) {
            ...
            .frame(
                width: StyleConstants.headerPickerIdealWidth,
                height: StyleConstants.headerModeToggleHeight
            )

            Spacer(minLength: 0)

            // Right: Use Renderfarm toggle
...
private struct RenderfarmToggle: View {
    ...
    var body: some View {
        Toggle(isOn: Binding(
            ...
            Text("Use Renderfarm")
                .font(.system(size: 11, weight: .semibold))
                .foregroundColor(isAvailable ? .primary : StyleConstants.headerInactiveLabel)
        }
        .toggleStyle(.switch)
        ...
        .help(
            ...
            ? "Toggle between Local and Renderfarm modes."
            : "Renderfarm unavailable; stay in Local mode."
        )
    }
}
...
private struct TabSelector: View {
    ...
    private func tabButton(title: String, tab: ModeTab, isActive: Bool) -> some View {
        Button {
            ...
            HStack(spacing: 4) {
                Image(systemName: tab == .manual ? "cursorarrow.click" : "bolt.fill")
                    .imageScale(.small)
