...
enum MetadataExtractor {
    ...
    static func extract(for url: URL) -> MediaMetadata {
        ...
        if let vTrack = asset.tracks(withMediaType: .video).first {
            ...
            if vTrack.minFrameDuration.isValid,
               ...
               vTrack.minFrameDuration.value != 0 {
                ...
                // As a last resort (may be 0 on some sources). This is deprecated on macOS 13+,
                // but only used when minFrameDuration can't give us a value.
        ...
        // ---- Timecode presence + attempt to get start TC -------------------
        out.hasTimecodeTrack = asset.tracks(withMediaType: .timecode).first != nil

        // Try QuickTime metadata (explicit "timecode" identifiers or likely info fields)
        ...
        if out.startTimecode == nil || out.nominalFPS == nil ||
            ...
            if let exif = ExifToolProbe.extract(for: url) {
                ...
                if out.colorPrimaries == nil    { out.colorPrimaries = exif.colorPrimaries }
                if out.transferFunction == nil  { out.transferFunction = exif.transferFunction }
                if out.ycbcrMatrix == nil       { out.ycbcrMatrix = exif.matrixCoefficients }
                if out.durationSeconds == nil   { out.durationSeconds = exif.durationSeconds }   // ← NEW
            }
...
// Inside MetadataExtractor.swift
...
enum ExifToolProbe {
    ...
    static func extract(for url: URL) -> Result? {
        ...
        // This version shells it directly; if you already have a run() helper, keep using it.
        ...
        let tcCandidates = [
            ...
            "Timecode",
            "Time Code",          // only if it looks like TC, not "2"
            "TimeCodeString",
            "StartTimecode",
            "TimeCode"
        ]
