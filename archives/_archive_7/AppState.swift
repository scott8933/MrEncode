...
// MARK: - Encode progress hooks (called by EncodeLocal / EncodeRemote)
extension AppState {

    /// Mark an item as currently encoding.
    func markEncoding(itemID: UUID) {
        DispatchQueue.main.async {
            guard let idx = self.files.firstIndex(where: { $0.id == itemID }) else { return }
            self.files[idx].status = .encoding
        }
    }

    /// Mark an item as finished and record the output path (if your MediaItem tracks it).
    func markFinished(itemID: UUID, outputURL: URL) {
        DispatchQueue.main.async {
            guard let idx = self.files.firstIndex(where: { $0.id == itemID }) else { return }
            self.files[idx].status = .done
            // If your MediaItem has a field for the rendered file, set it here.
            // e.g. self.files[idx].renderedURL = outputURL
            // (If not, safe to leave as just status change.)
        }
    }

    /// Mark an item as errored with a message (optional: store the message if your model has a field).
    func markError(itemID: UUID, message: String) {
        DispatchQueue.main.async {
            guard let idx = self.files.firstIndex(where: { $0.id == itemID }) else { return }
            self.files[idx].status = .error
            // If you track lastError on MediaItem, set it here:
            // self.files[idx].lastError = message
            print("Encode error for \(itemID): \(message)")
        }
    }
}
