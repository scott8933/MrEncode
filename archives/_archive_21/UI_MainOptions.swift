...
struct UI_MainOptions: View {
    ...
    private enum C {
        // Match Advanced/Deadline panel padding
        static let panelInsets = EdgeInsets(top: 6, leading: 6, bottom: 2, trailing: 6)
        static let splitterSpacing: CGFloat = 20
    }
        ...
        GroupBox {
            DisclosureGroup(isExpanded: $state.settings.generalExpanded) {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    // Quality + Scale row
                    HStack(spacing: C.splitterSpacing) {
                ...
                .padding(C.panelInsets) // identical to Advanced/Deadline

            } label: {
                // Match Advanced/Deadline header typography exactly
                Text("General Options").font(.headline)
            }
        }
    }
