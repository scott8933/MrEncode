...
struct UI_MessageArea: View {
    ...
    // MARK: Recent messages (newest first, limit as you prefer)
    private var recent: [AppLogEntry] {
        Array(state.uiMessages.suffix(3)).reversed()
    }
    ...
    var body: some View {
        if recent.isEmpty {
            ...
            let content = VStack(alignment: .leading, spacing: 6) {
                ForEach(recent) { message in
                    UI_MessageRowCompact(
                        ...
                        onCopy: {
                            copyToPasteboard(buildCopyText(from: message), entryID: message.id)
                        },
                        onRevealLog: {
            ...
            // Bottom-anchored, intrinsic height (no fixed/capped height)
            Group {
                content
                    .fixedSize(horizontal: false, vertical: true)
            }
            .frame(maxWidth: .infinity, alignment: .bottom)
            .padding(.bottom, StyleConstants.messageBottomMargin)
        }
    }
    ...
    private func copyToPasteboard(_ text: String, entryID: UUID) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + StyleConstants.copiedToastDuration) {
            ...
            if isExpanded {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    if let detail = entry.detail, !detail.isEmpty {
                        ...
                        .frame(maxHeight: 120)
                        .background(StyleConstants.panelFill)
                        .cornerRadius(6)
                        .overlay(
