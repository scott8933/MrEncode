    ...
    private static func encodeItem(_ item: MediaItem, settings: Settings, ffmpegPath: String) {
        ...
        DispatchQueue.main.async {
            ...
            if res.ok && finalizeOK {
                AppCore.shared.files[idx].finalOutputURL = outputURL
                AppCore.shared.files[idx].status = .done
                AppCore.shared.files[idx].statusReason = nil
                AppCore.shared.files[idx].actualEncodeSeconds = finishedAt.timeIntervalSince(startedAt)

                let secs = Int(round(AppCore.shared.files[idx].actualEncodeSeconds ?? 0))
                let mins = secs / 60
                let rem  = secs % 60
                ...
                learnForCompleted(item: AppCore.shared.files[idx],
                                  settings: AppCore.shared.settings,
                                  startedAt: startedAt,
                                  finishedAt: finishedAt)

                // Uncheck after success (matches prior behavior)
                ...
                AppCore.shared.files[idx].statusReason = finalizeError ?? "ffmpeg failed"
